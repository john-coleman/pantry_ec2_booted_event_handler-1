...
module Daemons
  class DnsCreateRecordCommandHandler
    ...
    def handle_message(message)
      ...
      when 'CNAME'
        runner.run_commands "dnscmd #{soa_server} /RecordAdd #{domain} #{hoestname} CNAME #{hostname}.#{domain}"
      else
        raise 'Invalid dns record type: #{record_type}'
      end
